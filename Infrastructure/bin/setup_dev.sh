...
oc start-build mlbparks --from-dir=MLBParks -n ${GUID}-parks-dev --follow  --wait
...
# Send binary build to OCP builder.
oc start-build nationalparks --from-dir=Nationalparks -n ${GUID}-parks-dev --follow --wait
# Always tag the newest version as latest, so there will be easy to manage DC and switching image when necessary.
...
# Send binary build to OCP builder.
oc start-build parksmap --from-dir=ParksMap -n ${GUID}-parks-dev --follow  --wait
# Always tag the newest version as latest, so there will be easy to manage DC and switching image when necessary
