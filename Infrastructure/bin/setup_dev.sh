...
oc new-build --name="mlbparks" --binary=true  jboss-eap70-openshift:1.7 --to=${GUID}-parks-dev/mlbparks:0.0-0 -e MAVEN_MIRROR_URL=http://nexus3.${GUID}-nexus.svc.cluster.local:8081/repository/maven-all-public -n ${GUID}-parks-dev
...
# Create new build and point to Nexus.
oc new-build --name="nationalparks" --binary=true  redhat-openjdk18-openshift:1.2 --to=${GUID}-parks-dev/nationalparks:0.0-0 -e MAVEN_MIRROR_URL=http://nexus3.${GUID}-nexus.svc.cluster.local:8081/repository/maven-all-public -n ${GUID}-parks-dev
# Send binary build to OCP builder.
...
oc policy add-role-to-user view --serviceaccount=default -n  ${GUID}-parks-dev
# Create new build and point to Nexus.
oc new-build --binary=true --name="parksmap" redhat-openjdk18-openshift:1.2 --to=${GUID}-parks-dev/parksmap:0.0-0 -e MAVEN_MIRROR_URL=http://nexus3.${GUID}-nexus.svc.cluster.local:8081/repository/maven-all-public -n ${GUID}-parks-dev
# Send binary build to OCP builder.
